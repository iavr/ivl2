...
template <typename T> struct tup_ref_t  : tx_refs_t <T, tup_val <T> > { };
...
template <typename... E> struct tup_ref_t <pack <E...> > : pack <E...> { };
template <typename T>    struct tup_ref_t <_type <T> >   : _type <T> { };
